...
class Astar {
     ...
     */
    checkNeighbours(x, y) {
        const g = this.points[x][y].g * squares[x][y];
        this.px = x;
        this.py = y;
        if (x - 1 >= 0) {
            if (y - 1 >= 0 && canDiagonally)
                this.calculateF(x - 1, y - 1, g + 14);
            if (y + 1 < h && canDiagonally)
                this.calculateF(x - 1, y + 1, g + 14);
            this.calculateF(x - 1, y, g + 10);
        }
        if (y - 1 >= 0)
            this.calculateF(x, y - 1, g + 10);
        if (y + 1 < h)
            this.calculateF(x, y + 1, g + 10);
        if (x + 1 < w) {
            if (y - 1 >= 0 && canDiagonally)
    ...
    calculateH(x, y) {
        ...
        switch (heuristic) {
            ...
            case 2:
                return parseInt(sqrt(sq(dx) + sq(dy)) * 10);
            case 3:
                return 0;
            default:
                return 0;
    ...
    drawText(x, y, override) {
        ...
        stroke(0);
        strokeWeight(0);
        text(this.getPointInfo(x, y), x * dim + 5, y * dim, dim / fraction);
    }
